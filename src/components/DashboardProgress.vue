<template>
  <div class="bg-primaryDark/30 border border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-5 mb-3 sm:mb-4 shadow-md">
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
      <div class="flex-1 h-6 sm:h-8 bg-primaryDark/50 rounded-full border border-white/10 relative overflow-hidden">
        <div
          class="h-full bg-gradient-to-r from-primarySecond via-primaryActive to-primaryDark rounded-full"
          :style="{ width: percent + '%' }"
        ></div>
      </div>

      <div class="px-3 sm:px-4 py-1 rounded-full border border-white/20 text-white font-bold text-xs sm:text-sm bg-primaryDark/50 text-center whitespace-nowrap">
        {{ days }}D
      </div>
    </div>
  </div>
